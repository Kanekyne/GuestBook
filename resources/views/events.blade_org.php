    ...
    <div class="container bg-light text-dark p-4">
        ...
        @else
            ...
            @if ($events->isEmpty())
                <p class="text-center text-warning">No hay eventos disponibles en este momento.</p>
            @endif
<p class="text-center">Cantidad de eventos cargados: {{ $events->count() }}</p>


            <p>Total de eventos recibidos: {{ $events->count() }}</p>
                ...
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <p class="card-text"><strong>Tipo:</strong> {{ $event->type }}</p>
                            <p class="card-text"><strong>Fecha de cierre:</strong> {{ $event->closed_at }}</p>
                            <a href="{{ url('/events/' . $event->_id . '/guestbook') }}" class="btn btn-primary">Ver libro
                                de visitas</a>
                        </div>
                    </div>
                </div>
